...
_replace_block          = _ou_mod.replace_marker_block
...
# _replace_block and _daily_note_template_fn are imported from obsidian_utils at module top.
...
def step_obsidian_daily_note(data: dict) -> str | None:
    ...
    existing = (
        ...
        if note_path.exists()
        else _daily_note_template_fn(DATE_STR)
    )
...
def step_update_obsidian_home(data: dict) -> None:
    ...
    home_path = OBSIDIAN_VAULT / "Adwi Home.md"
    ...
    daily_dir = OBSIDIAN_VAULT / "daily-notes"
    note_dates = sorted(
        [p.stem for p in daily_dir.glob("????-??-??.md")], reverse=True
    )[:7]
    notes_links = "\n".join(f"- [[daily-notes/{d}]]" for d in note_dates) if note_dates else "- (none yet)"

    # NLU status from CLAUDE.md
    nlu_line = "- NLU: (see [[knowledge/Eval and Reliability Map]])"
    try:
        claude_md = WORKSPACE / "CLAUDE.md"
        if claude_md.exists():
            for line in claude_md.read_text(encoding="utf-8").splitlines():
                if "**Current baseline" in line or "Combined (dedup)" in line:
                    nlu_line = f"- NLU: {line.strip().lstrip('|').strip()}"
                    break
    except Exception:
        pass
    ...
    if health.get("brew_outdated_count", 0) > 0:
        pending.append(f"  - brew upgrade ({health['brew_outdated_count']} packages)")
    if sk and sk not in ("[none]", "[Ollama offline — skipped]"):
        pending.append("  - AI skill suggestions (see daily note)")
    ...
    home_path.write_text(_replace_block(existing, "ADWI:HOME-STATUS", block), encoding="utf-8")


# ── Step 12: Update Pending Approval note ────────────────────────────────────

def step_update_obsidian_pending(data: dict) -> None:
    """Write generated pending-items block into obsidian-vault/knowledge/Pending Approval.md."""
    pending_path = OBSIDIAN_VAULT / "knowledge" / "Pending Approval.md"
    ...
    if health.get("brew_outdated_count", 0) > 0:
        items.append(
            f"- **brew upgrade** — {health['brew_outdated_count']} packages outdated. "
            "Run `brew outdated --verbose` to review, then `brew upgrade`."
        )
    if health.get("npm_outdated_count", 0) > 0:
        items.append(
            f"- **npm update -g** — {health['npm_outdated_count']} global package(s) outdated."
        )
    if sk and sk not in ("[none]", "[Ollama offline — skipped]"):
        items.append(
            f"- **AI skill suggestions** — review in [[daily-notes/{DATE_STR}]] "
            "under `### AI Skill Suggestions`. Apply with `/patch-adwi` or manually."
        )
    if heal.get("ran_tests") and not heal.get("tests_passed_after") and not heal.get("skipped_reason"):
        items.append(
    ...
    if items:
        block_body = f"*Last updated {DATE_STR} {TIME_STR}*\n\n" + "\n".join(items)
    else:
        block_body = f"*Last updated {DATE_STR} {TIME_STR}*\n\n- Nothing pending — system healthy ✓"

    existing = pending_path.read_text(encoding="utf-8")
    pending_path.write_text(_replace_block(existing, "ADWI:PENDING-APPROVAL", block_body), encoding="utf-8")
