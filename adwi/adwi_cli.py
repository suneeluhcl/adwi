...
_REGEX_INTENTS = [
    ...
    # Must come BEFORE CYCLE-1 so error reports mentioning blocked paths route to
    # fix_error (user asking for debugging help) rather than __none__ (security block).
    (re.compile(r"\bPermissionError\b.{0,15}\[Errno\s+\d+\]", re.I), "fix_error"),
    # "run aider" → patch_adwi — must come BEFORE self_heal's "fix adwi" pattern
    ...
    # Fires before file_read/file_list so blocked prompts get __none__ (explicit
    # block message) rather than falling through to PathValidator error at exec time.
    ...
    # ══ CYCLE-6: PRE-CHAT ORGANIZE GUARD — "best way to structure files" → organize ═
    # Must come BEFORE CYCLE-2a's broad "what's the best way to" → chat guard
    ...
    # Advisory "what's the best way to X" — reliably non-action
    ...
    # Frequency / best-practice advisory
    ...
    # FIX-CHAT-005: "what's the difference", compare-for-my, conversational continuations
    (re.compile(r"\bwhat's\s+the\s+difference\s+between\b", re.I), "chat"),
    (re.compile(r"\bcompare\b.{0,60}\bfor\s+my\b", re.I), "chat"),
    (re.compile(r"^\s*go\s+on\s*[.!?]?\s*$", re.I), "chat"),
    (re.compile(r"^\s*what\s+else\s*[?.]?\s*$", re.I), "chat"),
    ...
    # Adwi/AI concept questions (advisory, not action)
    ...
    (re.compile(r"^clean\s+up\s*$", re.I), "cleanup"),                   # bare "clean up" command
    ...
    # CYCLE-6: "find my heaviest files" — "heaviest" without "space" context
    ...
    # FIX-DU-002: typos "disk uzage", "dsk usage", "disk usafe"
    (re.compile(r"\bdisk\s+uz?[ae]?ge\b|\bdsk\s+usage\b", re.I), "disk_usage"),
    # FIX-DU-003: "storeage", "stoarge" typos for storage
    (re.compile(r"\bhow\s+much\b.{0,30}\bsto(?:re|ar)age\b", re.I), "disk_usage"),
    # FIX-DU-004: "am i running out of space", "is my disk almost full"
    (re.compile(r"\bam\s+i\s+running\s+out\b", re.I), "disk_usage"),
    (re.compile(r"\b(?:my\s+)?(?:disk|drive|ssd|hdd)\s+(?:is\s+)?(?:almost|nearly|nearly|getting)\s+full\b", re.I), "disk_usage"),
    ...
    # FIX-REL-004: "find backup scripts", "find test scripts" — scripts lookup
    ...
    # FIX-STATUS-003: "all ok?" / "is all ok?" — brief status probe (anchored to prevent false positives)
    ...
    # CYCLE-5: "nightly run" (noun-phrase order) → nightly_run
    ...
    # CYCLE-6: "switch to a local one" / "switch from cloud to local" → use_local
    ...
    # CYCLE-6: "show me the files in this email" — "files" + "in this email" signal
...
_INTENT_SYSTEM = (
    ...
    "                      NEVER for queries with 'notes', 'my notes', 'vault', or 'obsidian'\n"
    "                      combined with a search action — those are ALWAYS 'obsidian_search'.\n"
    "                      'search my notes about X'→obsidian_search. 'notes about X'→obsidian_search.\n"
...
def cmd_daily_brief(n8n_mode: bool = False) -> None:
    ...
    entry = f"# Daily Brief\n\n{brief}\n\n---\n\n{gmail_snippet}"
    _block_body = f"## Daily Brief\n*Last updated {now.strftime('%H:%M')}*\n\n{brief}"
    if gmail_snippet:
        _block_body += f"\n\n---\n\n{gmail_snippet}"

    _obs_saved = False
    # Try bridge first (preferred — consistent with /daily-note-update route)
    _bridge_result = _obsidian_api(
        "POST", "/daily-note-update",
        {"marker": "ADWI:DAILY-BRIEF", "content": _block_body},
    )
    if _bridge_result.get("daily_note"):
        _obs_saved = True
    else:
        # Fall back to direct file write
        _direct_ok, _direct_msg = _obsidian_daily_write_block("ADWI:DAILY-BRIEF", _block_body)
        if _direct_ok:
            _obs_saved = True
        else:
            if not n8n_mode:
                cprint(f"  {YELLOW}⚠ Obsidian write failed: {_direct_msg}{RESET}", "")

    if not n8n_mode:
        if _obs_saved:
            cprint(f"  {GREEN}✓ Saved to Obsidian daily note{RESET}", "")
        else:
            cprint(f"  {YELLOW}⚠ Could not save to Obsidian daily note{RESET}", "")
...
def _obsidian_daily_write_block(marker: str, block_body: str) -> tuple:
    """Write a marked block directly to today's vault daily note. Returns (success, message)."""
    try:
