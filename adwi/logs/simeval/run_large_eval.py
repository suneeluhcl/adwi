...
INTENT_SYSTEM = (
    ...
    "                      'explain what X.py does', 'explain the X module', 'inspect the X module'\n"
    "                      → inspect_code even if X = memory (memory.py). NOT 'memory_recall' (only Adwi's own stored facts).\n"
...
REGEX_INTENTS = [
    ...
    # FIX-006: "how do i benchmark it/my model" — verb form at end of sentence
...
def build_corpus() -> list[dict]:
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # ASSISTANT UPGRADE PACK — Phase 5 (12 scenarios)
    # ─────────────────────────────────────────────────────────────────────────
    for p in [
        "research latest MCP protocol changes",
        "do deep research on browser-use for AI agents",
        "save research about local AI assistants to my notes",
        "write a cited research brief on Ollama performance tuning",
        "deep dive into LangGraph multi-agent patterns",
    ]:
        add(p, "upgrade_pack", "research", "medium", fam="research")

    for p in [
        "browser task: go to ollama.com and find the latest release",
        "browser delegate: go to github.com/langchain-ai/langgraph and find the changelog",
        "browser agent: compare Exa and Firecrawl pricing pages dry run",
        "use browser to check the n8n release notes",
        "use playwright to visit the Qdrant docs and find vector index options",
    ]:
        add(p, "upgrade_pack", "browser_delegate", "medium", fam="browser_delegate")
    ...
    for p in [
        ...
        add(p, "upgrade_pack", "daily_brief", "easy", fam="daily_brief")

    for p in [
        "tech radar",
        "scan new AI tools for my setup",
        "what new AI frameworks should I try this week",
        "technology radar update",
    ]:
        add(p, "upgrade_pack", "tech_radar", "easy", fam="tech_radar")

    for p in [
        "curate memories from my recent notes",
        "memory curation",
        "propose new durable memories from my logs",
        "review my memory and suggest updates",
    ]:
        add(p, "upgrade_pack", "memory_curate", "medium", fam="memory_curate")

    for p in [
        "show assistant upgrade status",
        "upgrade pack status",
        "assistant upgrade status",
    ]:
        add(p, "upgrade_pack", "assistant_upgrade_status", "easy", fam="assistant_upgrade_status")
